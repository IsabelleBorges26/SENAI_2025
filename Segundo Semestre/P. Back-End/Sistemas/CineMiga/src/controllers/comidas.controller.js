...
const buscarComidas = async (req, res) => {
    ...
    try{
        const comida = await db.query("SELECT * FROM comida WHERE id = " + idComida);
    res.json(comida[0]).end();
    }catch(error){
        const info = {msg:""};

        if(error.errno === 1451) {
            info.msg = "Não é possível buscar esse conteúdo no momento.";
        }
    ...
    try{
         const novaComida = await db.query("INSERT INTO comidas VALUES (?, ?, ?)",[id, ingressos_id, pedido]);
         const comida = {
        id: id,
...
const atualizarComida = async (req, res) => {
    ...
    try{
        const atualiza =  await db.query("UPDATE comidas SET ingressos_id = ?, pedido = ? WHERE id = ? ", [id, ingressos_id, pedido]);
