...
const AtualizarIngresso = async (req,res) => {
    const {id, nome, data, filme} = req.body;

    try {
        const atuliza = await db.query("UPDATE ingressos SET nome = ?, data = ?, filme = ? WHERE id = ?", [nome, data, filme, id]);
        const info = {msg: ""};
...
const Listar_ingresso_id = async (req, res) => {
    const id = req.params.id;

    try {
        const BuscarIngresso = await db.query("SELECT * FROM ingressos WHERE id = ?", [id]);

        res.status(200).json(BuscarIngresso[0]).end();

    } catch (error) {
        const info = { msg: "" };

        if (error.errno === 1451) {
            info.msg = "Ingresso existente";
        }
        res.status(500).json(info).end();
    }
};

const IngressosVendidos = async (req, res) => {
    const nome = req.params.nome

    try {
        const Contar = await db.query ("SELECT COUNT(nome) AS Vendidos FROM ingressos WHERE filme = ?", [nome]);

        res.json(Contar[0]).end(); 
    } catch (error) {
        const info = { msg: "" };

        if (error.errno === 1451) {
            info.msg = "Total de ingressos vendidos";
        }
        res.status(500).json(info).end();
    }
};

const UltimaVenda = async (req, res) => {
    const nome = req.params.body

    try {
        const Ultima = await db.query ("SELECT MAX(data) AS Ultima_Venda FROM ingressos;");

        res.json(Ultima[0]).end(); 
    } catch (error) {
        const info = { msg: "" };
...
const Pedidos_Por_Ingresso = async (req, res) => {
    const ingressos_id = req.params.id;

    try {
        const PPI = await db.query(
            "SELECT COUNT(pedido) AS Pedidos_Por_Ingresso FROM comidas WHERE ingressos_id = ?",[ingressos_id]);
        res.json(PPI[0]).end();
    } catch (error) {
        const info = { msg: "" };

        if (error.errno === 1451) {
            info.msg = "Erro ao contar pedidos por ingresso";
        }
